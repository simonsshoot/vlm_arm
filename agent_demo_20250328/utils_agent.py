...
使用360视觉大模型定位物体并移动到该位置，比如：vlm_360_move('桌上有一个小人')
使用360视觉大模型定位物体并返回坐标（只定位不移动），比如：vlm_360_locate('桌上有一个小人')
使用360视觉大模型识别物体并用吸泵搬运，比如：vlm_360_pickup_and_move('把笔放在盒子上')
...
我的指令:先归零，再移动到小人的位置，然后摇头.你输出:{'function':['back_zero()', 'vlm_360_move("桌上有一个小人")', 'head_shake()'], 'response':'好的，我先回到原点，然后用我的火眼金睛找到小人并移过去，最后摇个头'}
我的指令:找到桌上的玩具车，移动到那里.你输出:{'function':['vlm_360_move("桌上的玩具车")'], 'response':'我马上用视觉大模型定位玩具车并移动过去'}
我的指令:桌上有一个药品盒子和一支笔，移动到这支笔位置上，使用吸泵吸取，并把笔放在盒子上.你输出:{'function':['vlm_360_pickup_and_move("把笔放在药品盒子上")'], 'response':'好的，我先找到笔和盒子，然后把笔搬到盒子上'}
我的指令:把笔放在肠溶胶囊药品盒子上.你输出:{'function':['vlm_360_pickup_and_move("把笔放在肠溶胶囊药品盒子上")'], 'response':'我马上帮你把笔搬到盒子上'}
